...
"""
Cerebras cloud data-collection script: runs Llama-3.1-8B (hosted on Cerebras's free tier) on the
Firsthand-Perspective materials.

Like the DeepSeek/Groq/HF scripts, this imports the scenario materials, scale items, persona
sampling, build_system_prompt(), and build_user_prompt() directly from firsthand.py -- the prompt
content is identical to the DeepSeek run; only the API target changes, keeping results comparable
across models.

Usage:
    pip install openai
    export CEREBRAS_API_KEY=your_key
    python firsthand_cerebras_llama.py --n_per_condition 200
"""
...
# Fieldnames are fixed in advance rather than inferred from the first successful record at
# runtime -- this avoids a record with an unexpectedly missing field locking in the wrong
# column set for all subsequent records (a hazard in an earlier version, now fixed).
...
# Cerebras free tier is rate-limited to 30 requests/minute; calls must be spaced at least 2.1s apart to avoid rate limiting
...
def call_model(system_prompt: str, user_prompt: str, model: str, temperature: float = 0.8,
               max_retries: int = 2, max_tokens: int = 2000) -> str:
    ...
    if _CEREBRAS_CLIENT is None:
        ...
        if not api_key:
            raise RuntimeError("Environment variable CEREBRAS_API_KEY not found; please run export CEREBRAS_API_KEY=your_key first")
        _CEREBRAS_CLIENT = OpenAI(api_key=api_key, base_url="https://api.cerebras.ai/v1")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if content is None:
                raise ValueError(f"Model returned empty content (likely the reasoning-token budget exhausted max_tokens, finish_reason={resp.choices[0].finish_reason})")
            return content
...
def _count_existing_by_condition(output_csv: str) -> dict:
    """Counts how many rows have already been saved per condition in the output file, for resume support."""
    import os
...
def run_collection(n_per_condition: int, model: str, output_csv: str, seed: int = 42, max_tokens: int = 2000):
    """Incremental CSV writing with fieldnames fixed in advance (not inferred from the first
    record); call spacing respects Cerebras rate limits. Supports resuming: if output_csv already
    exists, the number of completed rows per condition is counted first, and only the remaining
    gap is run."""
...
def quick_sanity_check(output_csv: str):
    ...
    print("\n=== Mean rating by condition x motive dimension (Llama-3.1-8B via Cerebras) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="firsthand_full_cerebras_llama3.1_8b.csv")
