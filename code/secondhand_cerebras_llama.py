...
"""
Cerebras cloud data-collection script: runs gpt-oss-120b (hosted on Cerebras) on the
Secondhand-Perspective materials (the "heard about it afterward" framing).

Key differences from firsthand_cerebras_llama.py:
...
- Scenario materials and persona sampling are imported from secondhand.py; scale items and
  build_user_prompt() are imported from firsthand.py (the same reuse pattern as secondhand.py's
  reuse for DeepSeek, just with call_model swapped for the Cerebras endpoint).
- Same resume support + hard-error (quota/balance exhausted) detection as
  firsthand_cerebras_llama.py.

Usage:
    pip install openai
    export CEREBRAS_API_KEY=your_key
    python secondhand_cerebras_llama.py --model gpt-oss-120b --n_per_condition 200
"""
...
def run_collection(n_per_condition: int, model: str, output_csv: str, seed: int = 42, max_tokens: int = 2000):
    """Incremental CSV writing with fieldnames fixed in advance; call spacing respects Cerebras
    rate limits. Supports resuming, plus hard-error (quota/balance exhausted) detection that
    terminates the whole run immediately."""
...
def quick_sanity_check(output_csv: str):
    ...
    print("\n=== Secondhand-Perspective: mean rating by condition x motive dimension (gpt-oss-120b via Cerebras) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="secondhand_full_cerebras_gptoss120b.csv")
