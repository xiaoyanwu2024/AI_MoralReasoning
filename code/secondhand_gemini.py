...
"""
Gemini data-collection script: runs Google Gemini 2.5 Pro on the Secondhand-Perspective materials
(the "heard about it afterward" framing).

Key differences from firsthand_gemini.py:
...
- Scenario materials and persona sampling are imported from secondhand.py; scale items and
  build_user_prompt() are imported from firsthand.py (identical to how firsthand_gemini.py calls
  Gemini; only the scenario materials change to the Secondhand-Perspective version).

Gemini 2.5 Pro has a "thinking" mechanism that, just as in the Firsthand-Perspective version,
consumes a hidden token budget, so max_tokens is likewise set to 4000, and an empty response on
failure is treated as an error rather than failing silently.

Usage:
    pip install openai
    export GEMINI_API_KEY=your_key
    python secondhand_gemini.py --n_per_condition 200
"""
...
def run_collection(n_per_condition: int, model: str, output_csv: str, seed: int = 42, max_tokens: int = 4000):
    """Incremental CSV writing + resume support + hard-error detection -- identical to firsthand_gemini.py."""
    import os
...
def quick_sanity_check(output_csv: str):
    ...
    print("\n=== Secondhand-Perspective: mean rating by condition x motive dimension (Gemini) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="secondhand_full_gemini25pro.csv")
