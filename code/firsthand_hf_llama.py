...
"""
Hugging Face free Inference API data-collection script: runs Meta-Llama-3.1-8B-Instruct
(the official weights, served via free serverless inference; unlike Groq, there is no
vendor-specific numeric compression, so this is closer to the "raw" model).

Reuses the scenario materials, scale items, persona sampling, and prompt-construction functions
from firsthand.py; only call_model() is replaced with the Hugging Face InferenceClient.

Lesson learned: an earlier version only wrote the output file once the whole run finished, and a
prior run hit HF's free-tier 402 paywall at 189/800 calls, losing all 189 already-generated rows.
This version has been changed to the same incremental writing + resume support + hard-error
(quota exhausted) detection mechanism used by the Cerebras/Gemini scripts, to avoid repeating that
mistake.

Usage:
    pip install huggingface_hub
    export HF_TOKEN=your_key
    python firsthand_hf_llama.py --n_per_condition 200
"""
...
def call_model(system_prompt: str, user_prompt: str, temperature: float = 0.8,
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_err = e
            time.sleep(2 * (attempt + 1))  # simple exponential backoff for free-tier rate limiting/queueing
...
def run_collection(n_per_condition: int, output_csv: str, seed: int = 42, max_tokens: int = 600):
    """Incremental CSV writing + resume support + hard-error detection -- identical to the Cerebras/Gemini scripts."""
...
def quick_sanity_check(output_csv: str):
    ...
    print("\n=== Mean rating by condition x motive dimension (Llama-3.1-8B-Instruct via HF Inference API) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="firsthand_full_hf_llama3.1_8b.csv")
